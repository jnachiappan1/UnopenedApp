...
const getTypeStyles = (rawType: string) => {
  ...
  const amountColor = type === 'debit' ? '#D00000' : '#38C36D';
...
const getTransactionDisplayInfo = (item: TransactionType) => {
  const {payment_transaction_type, amount, wallet_amount, transactionType} =
    item;
...
const TransactionCard: React.FC<Props> = ({item}) => {
  ...
  const breakdown = useMemo(() => {
    ...
    const hasWallet =
      walletAmt > 0 ||
      ['wallet_funds', 'add_funds'].includes(item?.payment_transaction_type);
    const shouldShow = hasWallet || externalAmt > 0;
    const externalLabel = item?.payment_type || item?.method || 'Stripe Card';
    const externalMasked = item?.meta?.card_last4
      ? `**** **** **** ${item?.meta?.card_last4}`
      : '';
    ...
    <View style={[styles.card, expanded && styles.cardExpanded]}>
        ...
        <View style={styles.amountContainer}>
          ...
          {breakdown.shouldShow && (
            <TouchableOpacity
              ...
              <IconsSvg
                name={expanded ? 'arrowUp' : 'downArrow'}
                width={16}
                height={16}
              />
            </TouchableOpacity>
      ...
      {expanded && breakdown.shouldShow && (
        <View style={styles.breakdownContainer}>
          ...
          {breakdown.walletAmt > 0 ||
          ['wallet_funds', 'add_funds'].includes(
            item?.payment_transaction_type,
          ) ? (
